...
export class AdminpolicylistComponent implements OnInit {
  ...
  userpolice:Userpolicy = new Userpolicy(0,0,0,"")
  ...
  updatestatus(userid:number){
   ...
   this.digiservice.myPolicyByUserId(userid).subscribe(data => this.userpolice = data)
    
  }

  updateUserpolicy(){
    alert("Admin response successfully")
    this.digiservice.updateuserpolicy(this.userpolice).subscribe(()=>{
